...
function optinrev_menu_icon(){
  ?><style>#adminmenu .toplevel_page_optinrevolution div.wp-menu-image:before {content: "\f116";}</style><?php
}
add_action( 'admin_head', 'optinrev_menu_icon' );
...
function optinrev_pro_get_started_headline()
{
  ...
  if (  in_array( $plugin_page, array('optinrevolution', 'optinrevolution/optin1', 'optinrevolution/optin-pro-settings') ) ) {
    ...
    if ( !isset($_COOKIE['OPTINREV_ALERT']) ) {
      echo __( '<div class="updated optinrev-alert" style="padding:6px;"><a href="#" class="optinrev-alert-close"></a>We have changed the way you add images. You can now add image via the tool bar. <br />These are the icons with the red arrows. The first icon with the red arrow is to change the action button and the second icon with the red arrow is to add an image.</div>' );
    }    
